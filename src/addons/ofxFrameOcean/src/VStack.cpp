...
vector<ofRectangle> VStack::get(ofRectangle frame_, float height_, float spacing_, float position_) {
    ...
    for (float y = frame_.getTop() + ofWrap(position_, -height_, spacing_) - (height_ + spacing_); y < frame_.getBottom(); y += height_ + spacing_) {
...
vector<ofRectangle> VStack::get(ofRectangle frame_, vector<float> weights_, float spacing_) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        frames.push_back(ofRectangle(frame_.x, y, frame_.width, ofClamp(height, 0, FLT_MAX)));
        y += height + spacing_;
    }
